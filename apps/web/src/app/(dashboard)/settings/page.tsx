...
export default function SettingsPage() {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    setPwLoading(true);
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000'}/auth/change-password`, {
        method: 'POST',
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="rounded-xl border border-border-soft bg-surface shadow-soft overflow-hidden">
            <div className="px-6 py-4 border-b border-border-soft bg-slate-50/50">
              <h2 className="text-[16px] font-semibold text-text-main flex items-center gap-2">
                <span className="material-symbols-outlined text-[20px] text-warning">lock</span>
                Change Password
              </h2>
            </div>
            <form onSubmit={handleChangePassword} className="p-6 space-y-4 max-w-md">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  required
                  className="h-10 px-3 rounded-lg border border-border-soft bg-background-light text-sm text-text-main focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/20 transition-colors"
                />
              </div>
              <div className="flex flex-col gap-1.5">
              ...
              {pwMessage && (
                <p className={`text-sm px-3 py-2 rounded-lg ${pwMessage.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}>
                  {pwMessage.text}
                </p>
              )}
              ...
              <button
                type="submit"
                disabled={pwLoading}
                className="h-10 px-5 rounded-lg bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors disabled:opacity-60"
              >
